...
//  @Test
	...
	public void testCreateShipVelocityNan() throws ModelException {
		...
		double velocity = Math.sqrt(facade.getShipVelocity(ship)[0] * facade.getShipVelocity(ship)[0] + facade.getShipVelocity(ship)[1] * facade.getShipVelocity(ship)[1]);
...
//      assertEquals(100, facade.getPlanetoidPosition(planetoid)[0], EPSILON);
	...
	public void testCreatePlanetoid() throws ModelException {
		if (nbStudentsInTeam > 1) {
			...
			assertEquals(4.0 * 0.917E12 * Math.PI * Math.pow(facade.getPlanetoidRadius(planetoid), 3) / 3.0, facade.getPlanetoidMass(planetoid), BIG_EPSILON);
...
//      assertTrue(facade.getBulletsOnShip(ship).contains(bullet2));
	...
	public void testLoadBulletOnShip() throws ModelException {
		...
		if (nbStudentsInTeam > 1) {
			assertEquals(1.0E40 + facade.getBulletMass(bullet1) + facade.getBulletMass(bullet2), facade.getShipMass(ship), VERY_BIG_EPSILON);
...
//  }
	...
	public void testEntityCollisionSlopingMovement() throws ModelException {
		max_score += 10;
		World world = facade.createWorld(5000, 5000);
		Ship ship1 = facade.createShip(100.0 / Math.sqrt(2.0), 100.0 / Math.sqrt(2.0), 10.0 / Math.sqrt(2.0), 10 / Math.sqrt(2.0), 20, 0, 1.0E20);
		Ship ship2 = facade.createShip(200.0 / Math.sqrt(2.0), 200.0 / Math.sqrt(2.0), -10 / Math.sqrt(2.0), -10 / Math.sqrt(2.0), 30, 0, 1.0E20);
		facade.addShipToWorld(world, ship1);
		facade.addShipToWorld(world, ship2);
		double dt = facade.getTimeCollisionEntity(ship1, ship2);
		assertEquals(2.5, dt, 0.1);
...
//    Ship ship1 = facade.createShip(100, 100, -10, -10, 50, 0, 1.0E20);
    ...
    System.out.print("het moederschip is: ");
